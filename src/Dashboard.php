...
<style>
  :root {
    --primary-color: #FFCCD8;
    --primary-hover: #f7b5c6;
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
      <div class="lg:col-span-2 bg-white shadow-md rounded-lg p-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex space-x-2">
            <button id="openFormBtn" aria-label="Tambah jadwal baru" class="bg-[var(--primary-color)] hover:bg-[var(--primary-hover)] text-gray-800 font-bold py-2 px-4 rounded-md flex items-center transition-colors duration-300">
              <i class="fas fa-plus mr-2"></i> Tambah
            </button>
            <button id="lihatSemuaJadwalBtn" class="bg-gray-200 hover:bg-gray-300 text-[#b24b65] font-semibold px-4 py-2 rounded-md border border-gray-300">Lihat Semua</button>
          </div>
        </div>
                ...
                <?php foreach ($jadwalList as $jadwal): ?>
                  <tr class="bg-white border-b hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <td class="py-4 px-4 text-center w-28">
                      <button class="text-[var(--primary-color)] hover:text-[var(--primary-hover)] p-2 editBtn"
      ...
      <div class="lg:col-span-1 space-y-6">
        <div onclick="window.location.href='Index.php?page=distribusi';" class="bg-white shadow-md rounded-lg p-5 flex items-center cursor-pointer hover:shadow-lg transition-shadow duration-300">
          <div class="bg-[var(--primary-color)] text-gray-800 rounded-full p-4 mr-4">
...
<div id="modalJadwalLengkap" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
  <div class="bg-white max-w-5xl w-full rounded-lg shadow-lg p-6 overflow-y-auto max-h-[90vh] relative">
    <h3 class="text-xl font-bold mb-4 text-[var(--primary-hover)]">Jadwal Lengkap</h3>
...
<div id="modalOverlay" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
  <div class="bg-white shadow-xl rounded-lg p-6 w-full max-w-md relative">
    <form id="jadwalForm" action="Index.php?page=dashboard" method="POST">
      <h2 id="formTitle" class="text-xl font-bold text-gray-800 mb-6">Input Jadwal</h2>
      <input type="hidden" name="id_jadwal" id="id_jadwal">
      <div class="space-y-4">
        <div>
          <label for="tanggal" class="block text-sm font-medium text-gray-700 mb-1">Tanggal</label>
          <input type="date" id="tanggal" name="tanggal" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--primary-hover)]" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Waktu</label>
          <div class="flex space-x-2 items-center">
            <input type="time" id="jam_mulai" name="jam_mulai" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--primary-hover)]" />
            <span class="text-gray-500">-</span>
            <input type="time" id="jam_selesai" name="jam_selesai" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--primary-hover)]" />
          </div>
        </div>
        <div>
          <label for="jenis_kegiatan" class="block text-sm font-medium text-gray-700 mb-1">Jenis Kegiatan</label>
          <select id="jenis_kegiatan" name="jenis_kegiatan" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--primary-hover)]">
      ...
      <div class="flex justify-end space-x-3 mt-8">
        <button type="button" id="cancelBtn" class="bg-gray-200 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-300">Batal</button>
        <button type="submit" name="submit_jadwal" class="bg-[var(--primary-color)] text-gray-800 px-4 py-2 rounded-md hover:bg-[var(--primary-hover)]">Simpan</button>
      </div>
    </form>
  </div>
</div>
...
<div id="deleteDialog" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
  <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm">
    ...
    <form action="Index.php?page=dashboard" method="POST" class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
      ...
      <button type="button" id="cancelDeleteBtn" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm">
        Batal
      </button>
    </form>
  </div>
