    ...
    <form method="GET" class="flex flex-wrap gap-4 items-end mb-6">
      ...
      <div class="flex items-center gap-2">
        <a href="<?= htmlspecialchars($pdfUrl) ?>" target="_blank" rel="noopener"
          class="px-4 py-2 rounded inline-flex items-center mt-1 text-white font-medium"
          style="background-color:#800020; transition:background-color .2s ease;"
          onmouseover="this.style.backgroundColor='#a3323b';"
          onmouseout="this.style.backgroundColor='#800020';"
          title="Cetak / Simpan PDF">Cetak PDF</a>
      </div>
    </form>
    ...
    <?php if ($kategori==='semua'): ?>
      <div class="overflow-x-auto">
        <table class="min-w-full border text-sm text-left bg-white">
          <thead style="background-color:#FFCCD8" class="text-gray-900">
            <tr>
              <th class="border px-3 py-2">Periode</th>
              <th class="border px-3 py-2">Produksi</th>
              <th class="border px-3 py-2">Produk Jual</th>
              <th class="border px-3 py-2">Distribusi</th>
              <th class="border px-3 py-2">Stok</th>
              <th class="border px-3 py-2">Gaji Dibayar</th>
              <th class="border px-3 py-2">Pekerja Aktif</th>
              <th class="border px-3 py-2">Pesanan</th>
            </tr>
          </thead>
          <tbody>
            <?php foreach ($periodRows as $r): ?>
              <tr class="hover:bg-[#FFE9F0]">
            ...
            <?php if(empty($periodRows)): ?>
              <tr><td colspan="8" class="text-center text-gray-500 py-3">Tidak ada data.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
